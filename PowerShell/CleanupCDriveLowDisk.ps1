...
.EXAMPLE 
   PS C:\Users\kumarsat\Desktop\Powershell> .\cleanup_log.ps1 
...
## Remove all files and folders Prefetch.
Get-ChildItem "C:\Windows\Prefetch\*.* /Q" 
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete))} | 
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue 
...
Where-Object { ($_.CreationTime -le $(Get-Date).AddDays(-10)) } | 
Remove-Item -Force -Verbose -Recurse -ErrorAction SilentlyContinue 
